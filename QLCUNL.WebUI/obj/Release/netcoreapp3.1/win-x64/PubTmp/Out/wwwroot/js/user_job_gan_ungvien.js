                ...
                res.data.forEach(item => {
                    ...
                    if (item.thuoc_tinh != null) {
                        item.thuoc_tinh.forEach(tt => {
                            html_thuoc_tinh += `<span class="font-weight-bold">${tt.ten}</span><br>`;
                        });
                    }

                    html_data += `<td>${html_thuoc_tinh}</td>`;
                    html_data += `<td>${item.dia_chi}</td>`;
                    html_data += `<td>${item.vi_tri_ung_tuyen}</td>`;
                    html_data += `<td>${item.ghi_chu_cuoi}</td>`;
                    html_data += `<td><button class="btn-size btn btn-primary" onclick="onOneAssign('${item.id_ung_vien}')">Gán</button></td>`;
                    html_data += `</tr>`;
                });
    ...
    callAPI(`${API_URL}/UngVien/view?id=${id}`, null, "GET", function (res) {
        if (res.success) {
            if (res.data != null && res.data != undefined && res.data != "") {
                ...
                try {
                    let ext = res.data.link_cv.split('.').pop();
                    let url_temp = ext == "pdf" ? res.data.link_cv : "https://view.officeapps.live.com/op/view.aspx?src=" + res.data.link_cv;
                    $("#ungvien_detail").append(`<li>Link CV: <span class="font-weight-bold"><a target='_blank' href='${url_temp}'>${res.data.link_cv}</a></span></li>`);
                } catch (e) {
                }
