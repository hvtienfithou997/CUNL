    ...
    callAPI(`${API_URL}/baocao/view?ngay_nhan_job_from=${ngay_nhan_job_from}&ngay_nhan_job_to=${ngay_nhan_job_to}&thuoc_tinh=${thuoc_tinh}&gop_job=false`, null, "GET", function (res) {
        if (res.success) {
            ...
            res.data.forEach(item => {
                ...
                html += `<td>${formatCurency(item.so_tien_da_nhan)}</td>`;
                html += `<td>${formatCurency(item.so_tien_con_phai_thanh_toan)}/${formatCurency(item.so_tien_con_phai_thanh_toan_du_kien)}</td>`;
