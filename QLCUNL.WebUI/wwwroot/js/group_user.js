    ...
    callAPI(`${API_URL}/groupuser/search?term=${term}&page=1`, null, "GET", function (res) {
        if (res.success && res.data != null) {
            ...
            res.data.forEach(item => {
                ...
                html += `<td><a class="btn btn-warning btn-rieng" href="edit?id=${item.id}">Sửa</a>`;
                html += `<button class="btn btn-danger btn-rieng" id="${item.id}" onclick="deleteRec(this.id)">Xóa</a></td>`;
                html += `</tr>`;
            });
            $("#div_data").html(html);
        }
